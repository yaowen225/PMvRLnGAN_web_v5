...
- [十、測試指南（給用戶）](#十測試指南給用戶) - 階段性測試流程、常見問題排查
...
> **重要說明**：本測試指南專為您（用戶）設計，目的是指導您如何測試每次更新後的功能。每次開發階段完成後，您應按照以下步驟進行測試，並將發現的問題反饋給開發者。

### 10.1 階段性測試流程

每個開發階段完成後，您應進行系統測試以確保功能正常運作。以下是測試流程的詳細說明：

#### 10.1.1 環境準備

1. **確認工作目錄**：
   - 您應在項目根目錄 `PMvRLnGAN_web` 下執行所有命令
...
> **重要說明**：本節描述了開發者和用戶之間的協作流程，確保開發過程的順暢和文檔的完整性。無論是否有前面的對話記錄，都應遵循此協作方式。
...
通過遵循這些協作方式，我們可以確保開發過程的透明度和效率，同時保持文檔的完整性和準確性，即使沒有前面的對話記錄，也能按照此方式順利進行協作。 